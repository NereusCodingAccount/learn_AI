...
#### Since using a step size **_n_** that is too small would slow convergence, and a n too large would lead to overshoot(過衝） and divergence, finding a good setting of **_n_** is an important practical problem. Philip Wolfe also advocated(提倡） using **"clever(聰明的） choices of the [ descent ] direction"** in practice (10) While using a direction that deviates(偏離） from the steepest descent direction may seem counter-intuitive, the idea is that the smaller slope(坡） may be compensated(補償） for by being sustained(持續） over a much longer distance.
#### To reason about this mathematically, consider(考慮） a direction **_Pn_** and step size in and consider the more general update:
#### Finding good settings of **_Pn_** and **_n_** requires(要求） some thought(思維）. First of all, we would like the update direction to point downhill(下坡）. Mathematically(數學上）, letting On denote(表示） the angle between **_- Vf(an)_** and **_Pn_**, this requires that **_cos On > 0_**. To say more, we need more information about the objective function that we are optimising(最佳化）. Under the fairly(相當） weak assumption(架假設） that fis(在） continuously(持續） differentiable(可微分的）, we may prove(證明） that:
